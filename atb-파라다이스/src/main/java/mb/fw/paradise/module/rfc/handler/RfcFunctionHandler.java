...
import mb.fw.paradise.common.constants.ESBCommonFieldConstants;
...
public class RfcFunctionHandler implements JCoServerFunctionHandler {
	...
	public void handleRequest(JCoServerContext ctx, JCoFunction function) {
		...
		MetaApiModel interfaceInfo = null;
		...
		try {
			...
			// 1) 인터페이스 정보 조회
			List<MetaApiModel> interfaceInfoList = loadInterfaceInfoList(functionName);
	...
	private List<MetaApiModel> loadInterfaceInfoList(String functionName) {
		List<MetaApiModel> list = apiService.getInterfaceInfoByFunctionName(functionName, interfaceIdList).block();
	...
	private MetaApiModel selectInterface(JCoFunction function, List<MetaApiModel> interfaceList) throws Exception {
		String targetSysCode = extractTargetSysCode(function);
		log.info("targetSysCode=[{}], RFC={}", targetSysCode, function.getName());
		for (MetaApiModel info : interfaceList) {
	...
	private DataItem extractImportData(JCoFunction function, MetaApiModel interfaceInfo) {
	...
	private APIResponseMessage callGateway(MetaApiModel interfaceInfo, String txId, DataItem dataItem, int dataCount) {
	...
	private void exportToSapSuccess(JCoFunction function, APIResponseMessage response, String txId, MetaApiModel info)
			throws Exception {
	...
	private void exportToSapError(JCoFunction function, Exception e, String txId, MetaApiModel info) {
		try {
	...
	private void fillStructureExport(JCoFunction function, MetaApiModel info, String txId, String message,
			String status) {
		if (info == null)
			return;
		List<PatternProperty> propertyList = new ArrayList<>(info.getPropertyList());
	...
	private void fillTableExport(JCoFunction function, MetaApiModel info, APIResponseMessage response)
			throws Exception {
